...
const ConnectedArticle = (props) => {
  ...
  const rename = async (e) => {
    ...
    await askGraphQL(props.endpoints.graphql, { query, variables }, 'Renaming Article')
    setTitle(tempTitle)
    setRenaming(false)
  }
  ...
  return (
    <article>
      ...
      <nav>
        <Bouton title="Preview" href={`https://via.hypothes.is/${props.endpoints.export}/api/v1/htmlArticle/${props._id}?preview=true`}>
          <Icon.Eye/>
        </Bouton>
      ...
      <p style={{ paddingLeft: "1rem" }}>
        {tags.map(t => <span key={"tagColor-" + t._id} style={{
          fontSize: "0.6rem",
      ...
      {expanded && <section>
        <ul>
          ...
          {props.versions.map(v => (
            <li key={`version-${v._id}`}><Link
              to={`/article/${props._id}/version/${v._id}`}>{`${v.message ? v.message : 'no label'} (${v.autosave ? 'autosaved' : ''} v${v.version}.${v.revision})`}</Link></li>
          ))}
        </ul>
        ...
        <ul>
          ...
          }}>{editTags ? 'finish' : 'edit'}</span>):</p>
          <ArticleTags editTags={editTags} {...props} stateTags={tags} setTags={(ts) => setTags(ts)}/>
        </ul>
        {deleting && <div className={styles.alert}><p>You are trying to delete this article, double click on the "delete button" below to proceed</p>
          <button className={styles.cancel} onClick={() => setDeleting(false)}>Cancel</button>
          <ArticleDelete {...props}/></div>}
      </section>}
    </article>
  )
}
