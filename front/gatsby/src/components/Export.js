...
export default props => {
  ...
  const startExport = (endpoints) => {
    const processEndpoint = endpoints.process
    const exportEndpoint = endpoints.export
    if (props.book) {
      //For books
      window.open(`${processEndpoint}/cgi-bin/exportBook/exec.cgi?id=${filterAlphaNum(props.name)}&book=${props.bookId}&processor=xelatex&source=${exportEndpoint}/&format=${format}&bibstyle=${csl}&toc=${toc}&tld=${tld}&unnumbered=${unnumbered}`, '_blank');
    } else {
      //For articles/versions
      window.open(`${processEndpoint}/cgi-bin/exportArticle/exec.cgi?id=${filterAlphaNum(props.title)}v${props.version}-${props.revision}&version=${props.versionId}&processor=xelatex&source=${exportEndpoint}/&format=${format}&bibstyle=${csl}&toc=${toc}`, '_blank');
    }
  }
  ...
  return (
    <section className={styles.export}>
      ...
      <form>
        ...
        {props.book && <select value={unnumbered} onChange={(e) => setUnnumbered(etv(e))}>
      ...
      <nav>
        <p onClick={() => startExport(props.endpoints)}>Export</p>
      </nav>
    </section>
  )
}
