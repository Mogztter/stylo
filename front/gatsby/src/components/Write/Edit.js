  ...
  return (
    <section id={styles.section}>
      {exporting && <Modal cancel={() => setExporting(false)}>
        <Export {...exportVar} endpoints={props.endpoints}/>
      </Modal>}
      ...
      {expand && <>
        ...
        {!props.readOnly && <nav>
          ...
          <a href={`https://via.hypothes.is/${props.endpoints.export}/api/v1/htmlArticle/${props.article._id}?preview=true`}
             target="_blank" rel="noopener noreferrer"
             className={styles.button}>preview</a>
        </nav>}
