...
const ConnectedUser = props => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await askGraphQL(props.endpoints.graphql, { query, variables }, 'Fetching user')
  ...
  const unlinkZoteroAccount = async () => {
    ...
    setIsLoading(true)
    const data = await askGraphQL(props.endpoints.graphql, { query, variables }, 'Clear Zotero token')
  ...
  const updateInfo = async (e) => {
    ...
    try {
      ...
      const data = await askGraphQL(props.endpoints.graphql, { query, variables }, 'Updating user')
  ...
  const addNewLogin = async (e) => {
    ...
    try {
      ...
      const data = await askGraphQL(props.endpoints.graphql, { query, variables }, 'Adding password to user')
      setEmailLogin('')
  ...
  const removeLogin = async (email, _id) => {
    try {
      ...
      const data = await askGraphQL(props.endpoints.graphql, { query, variables }, 'Removing password to user')
  ...
  return (
    <section className={styles.section}>
      ...
      <form onSubmit={(e) => updateInfo(e)}>
        ...
        <label>Zotero:</label><p>{user.zoteroToken ? <span>Linked with <b>{user.zoteroToken}</b> account.</span> : <span>No linked account.</span>}{user.zoteroToken &&
      <button title="Unlink this Zotero account" onClick={(e) => e.preventDefault() || unlinkZoteroAccount()}>Unlink</button>}</p>
