...
const ConnectedCreateTag = (props) => {
  ...
  const createTag = async (event, cb, query, variables, token) => {
    try {
      ...
      await askGraphQL(props.endpoints.graphql, { query, variables }, 'Creating new tag')
      cb()
    } catch (err) {
      alert(err)
    }
  ...
  return (
    <section className={styles.create}>
      <form onSubmit={(event) => {
        createTag(event, props.triggerReload, query, variables, props.sessionToken)
      }}>
        ...
        <ul>
          ...
          {articlesSelected.map(a => (
            <li key={`selectArticle-${a._id}`} className={a.selected ? styles.selected : styles.unselected}
                onClick={() => setArticlesSelected(findAndUpdateArticle(articlesSelected, a._id))}>{a.title}</li>
